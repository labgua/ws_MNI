 ...
 *	la somma di due
 *	matrici quadrate di dimensione N.
 *	Memorizzare le matrici in array monodimensionali.
 *	Configurare il kernel come una griglia bidimensionale di DxD blocchi, con
 *	blocchi bidimensionali di NtxNt threads, con Nt=8,16 e 32.
 *
 **/
...
int main(int argn, char* argv[]){
	...
	printf("***\t SOMMA DI DUE MATRICI, griglia: D x D  e blockDim: th x th  \t***\n");
	printf("Inserisci N num righe e colonne : ");
	scanf("%d", &N);
	printf("Inserisci la dimensione (quadrata) del threadBlock [8,16,32]: ");
	scanf("%d", &nth);
